...
class IntervalSnipper(Snipper):
    ...
    prevOcrResult = None
    timer = None  # Initialize timer at the class level
    ...
    def onShotOcrInterval(self):
        ...
        if prev_ocr_result == ocr_result:
            ...
            pyperclip.copy(ocr_result)
            log_copied(ocr_result)
            self.stopTimer()  # Stop the timer after copying to clipboard
...
def take_textshot(langs, interval):
    ...
    if interval is not None:
        snipper = IntervalSnipper(window, interval, langs + "+hin")
        snipper.show()
    else:
